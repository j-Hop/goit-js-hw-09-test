...
(0, _flatpickrDefault.default)(dataTimePickerUrl, options);
counterStartBtnUrl.addEventListener("click", onCounterStart);
let isIntervalActive = false;
function onCounterStart() {
    if (isIntervalActive) {
        (0, _notiflixNotifyAio.Notify).warning("The countdown has already started! Please, reload the page.");
        return;
    }
    (0, _notiflixNotifyAio.Notify).success("The countdown has begun.");
    const intervalId = setInterval(()=>{
        let preventTimerResult = selectedDate - new Date().getTime();
        let convertedTimerResult = convertMs(preventTimerResult);
        urls.timerDaysSpan.textContent = addLeadingZero(String(convertedTimerResult.days));
        urls.timerHoursSpan.textContent = addLeadingZero(String(convertedTimerResult.hours));
        urls.timerMinutesSpan.textContent = addLeadingZero(String(convertedTimerResult.minutes));
        urls.timerSecondsSpan.textContent = addLeadingZero(String(convertedTimerResult.seconds));
        if (preventTimerResult < 1000) clearInterval(intervalId);
    }, 1000);
...
},{}],"eVN6V":[function() {},{}]},["awmoY","1AWyZ"], "1AWyZ", "parcelRequired7c6")
